...
class GenerationService:
    """
    生成服务类：负责调用不同的模型提供商（HuggingFace、OpenAI、DeepSeek、SiliconFlow）生成回答
    ...
    def _generate_with_siliconflow(
        ...
        参数:
            ...
            query: 用户查询
            context: 上下文信息
            api_key: SiliconFlow API密钥，如不提供则从环境变量获取
        ...
        try:
            ...
            # 获取实际模型名称
            model_id = self.models["siliconflow"][model_name]
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "你是一个有帮助的助手，请根据提供的上下文回答问题。"
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            logger.error(f"HTTP error generating with SiliconFlow: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Error generating with SiliconFlow: {str(e)}")
            raise
    ...
    def generate(
        ...
        参数:
            provider: 模型提供商，可选值为"huggingface"、"openai"、"deepseek"、"siliconflow"
            model_name: 模型名称
            query: 用户查询
        ...
        try:
            ...
            if provider == "huggingface":
                ...
                response = self._generate_with_siliconflow(model_name, query, context, "sk-cieanfgxijrnpjwcryoacvulkmddronmgetnogpblipjrwhn")
            else:
